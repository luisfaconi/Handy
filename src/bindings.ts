...
async isMeetingModeSupported() : Promise<boolean> {
    return await TAURI_INVOKE("is_meeting_mode_supported");
},
async getMeetingModeState() : Promise<boolean> {
    return await TAURI_INVOKE("get_meeting_mode_state");
},
async startMeetingMode() : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("start_meeting_mode") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async stopMeetingMode() : Promise<Result<string | null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("stop_meeting_mode") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async openMeetingFile(fileName: string) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("open_meeting_file", { fileName }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type EntryType = "normal" | "meeting"
export type GpuDeviceOption = { id: number; name: string; total_vram_mb: number }
export type HistoryEntry = { id: number; file_name: string; timestamp: number; saved: boolean; title: string; transcription_text: string; post_processed_text: string | null; post_process_prompt: string | null; post_process_requested: boolean; entry_type: EntryType; processing_status?: "starting" | "processing" | "completed" }
